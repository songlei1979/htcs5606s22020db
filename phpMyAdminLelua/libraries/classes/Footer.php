...
class Footer
{
    ...
    {
        $message = '<a href="/">' . __('phpMyAdminLei Demo Server') . '</a>: ';
    ...
    private function _getSelfLink(string $url): string
    {
        ...
        $retval .= '<a href="' . htmlspecialchars($url) . '"'
            . ' title="' . __('Open new phpMyAdminLei window') . '" target="_blank" rel="noopener noreferrer">';
        if (Util::showIcons('TabsMode')) {
            $retval .= Util::getImage(
                'window-new',
                __('Open new phpMyAdminLei window')
            );
        } else {
            $retval .=  __('Open new phpMyAdminLei window');
        }
        $retval .= '</a>';
        $retval .= '</div>';
        return $retval;
