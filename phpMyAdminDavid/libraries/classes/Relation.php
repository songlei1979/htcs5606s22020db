...
class Relation
{
    ...
    public function getRelationsParamDiagnostic(array $cfgRelation)
    {
        ...
        if (strlen((string) $cfgRelation['db']) == 0) {
            ...
            if (! $cfgRelation['allworks']) {
                ...
                $items[] = __(
                    'Re-login to phpMyAdminLei to load the updated configuration file.'
                );
        ...
        if ($error = $this->dbi->getError()) {
            if ($GLOBALS['errno'] == 1044) {
                $GLOBALS['message'] =    __(
                    ...
                    . ' database to set up the phpMyAdminLei configuration storage there.'
                );
            } else {
    ...
    public function getHtmlFixPmaTables($allTables, $createDb = false)
    {
        ...
        if ($allTables) {
            if ($createDb) {
                ...
                $message = Message::notice(
                    __(
                        ...
                        . 'the phpMyAdminLei configuration storage there.'
                    )
                );
            } else {
                ...
                $message = Message::notice(
                    __(
                        '%sCreate%s the phpMyAdminLei configuration storage in the '
                        . 'current database.'
                    )
                );
            }
            ...
            $message = Message::notice(
                __('%sCreate%s missing phpMyAdminLei configuration storage tables.')
            );
        }
